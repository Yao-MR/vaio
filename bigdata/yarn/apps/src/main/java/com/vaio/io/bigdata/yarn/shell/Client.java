...
public class Client {
  ...
  public Client(Configuration conf) throws Exception  {
    this(
            "com.vaio.io.bigdata.yarn.shell.ApplicationMaster",
            conf);
  }
  ...
  public boolean run() throws IOException, YarnException {
    ...
    LOG.info("1 拿到集群的node的个数"+ clusterMetrics.getNumNodeManagers());

    List<NodeReport> clusterNodeReports = yarnClient.getNodeReports(
            NodeState.RUNNING);
    LOG.info("2 集群每个节点的的状态报告");
    ...
    LOG.info("拿到队列的信息是"
            + ", 队列的名字是=" + queueInfo.getQueueName()
            + ", 队列现在的容量是=" + queueInfo.getCurrentCapacity()
            + ", 队列的最大容量是=" + queueInfo.getMaximumCapacity()
            + ", 队列的应用程序的个数是=" + queueInfo.getApplications().size()
            + ", 队列的子队列的个数是=" + queueInfo.getChildQueues().size());

    List<QueueUserACLInfo> listAclInfo = yarnClient.getQueueAclsInfo();
    for (QueueUserACLInfo aclInfo : listAclInfo) {
      for (QueueACL userAcl : aclInfo.getUserAcls()) {
        LOG.info("User ACL Info for 队列的权限是"
    ...
    LOG.info("集群的最大的虚拟核心的数量是 " + maxVCores);
    ...
    LOG.info("开始配置未来appMaster的运行环境配置");
    ...
    LOG.info("开始构造appMaster 启动的命令,这个命令与将来子在node上运行的命令是一样的");
    ...
    LOG.info("构造appmaster 运行的命令已经完成 " + command.toString());
    ...
    LOG.info("现在使用yarn的客户端把我们app的上下提交给rm-------------------------------------------------------");
